...
class _LazyMapping(OrderedDict):
    ...
    def __getitem__(self, key: str) -> Type[Any]:
        ...
        if key not in self._mapping:
            raise KeyError(f"'{key}' not found. Available keys: {list(self._mapping.keys())}")
        ...
        if module_name not in self._modules:
            try:
                self._modules[module_name] = importlib.import_module(f".{module_name}", "memories")
            except ImportError as e:
                raise ImportError(f"Failed to import {module_name} for {key}: {e}")
    ...
    def register(self, key: str, value: Type[Any], exist_ok: bool = False) -> None:
        ...
        if key in self._mapping and not exist_ok:
            raise ValueError(
                f"'{key}' is already registered in {self._mapping_type} mapping. Use exist_ok=True to overwrite."
            )
