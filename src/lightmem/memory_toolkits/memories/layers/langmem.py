...
class LangMemLayer(BaseMemoryLayer):
    ...
    def __init__(self, config: LangMemConfig) -> None:
        ...
        self._query_model = None if config.query_model is None else init_chat_model(config.query_model)
            ...
            outputs.append(
                {
                    ...
                    "metadata": {key: value for key, value in memory_dict.items() if key != "value"},
    ...
    def update(self, memory_id: str, **kwargs) -> bool:
        ...
        try:
            self.memory_layer.put(memory_id, {"content": content}, **kwargs)
            return True
        except Exception as e:
    ...
    def save_memory(self) -> None:
        ...
        # Write config.json
        config_path = os.path.join(self.config.save_dir, "config.json")
        config_dict = {"layer_type": self.layer_type, **self.config.model_dump()}
