...
class MemZeroLayer(BaseMemoryLayer):
    ...
    def add_message(self, message: Dict[str, str], **kwargs) -> None:
        ...
        try:
            self.memory_layer.add(
                messages=message["content"], user_id=self.config.user_id, metadata={"timestamp": timestamp}
            )
            ...
            is_content_filter_error = (
                "content management policy" in msg or "content_filter" in msg or "'messages' must be list[dict]" in msg
            )
    ...
    def add_messages(self, messages: List[Dict[str, str]], **kwargs) -> None:
        ...
        self.memory_layer.add(messages=messages, user_id=self.config.user_id)
    ...
    def retrieve(self, query: str, k: int = 10, **kwargs) -> List[Dict[str, Union[str, Dict[str, Any]]]]:
        try:
            ...
            is_content_filter_error = (
                ...
                or "upstream_error" in msg
                and "param': 'prompt'" in msg
            )
        ...
        for item in results:
            ...
            used_content_str = "\n".join(f"{kk}: {vv}" for kk, vv in used_content_dict.items() if vv is not None)
    ...
    def save_memory(self) -> None:
        ...
        try:
            ...
            all_memories = self.memory_layer.get_all(user_id=self.config.user_id, limit=100000)
