...
class NonCachedLLMOperator(ABC):
    def __init__(self, prompt_name: str, model_name: Optional[str] = None, **kwargs) -> None:
        params = inspect.signature(self._preprocess).parameters
        for param in params:
            if not param.endswith("_list"):
                raise ValueError("The name of each argument in the `preprocess` function must end with `_list`.")
    ...
    def _check_prompt_identifiers(self) -> bool:
        params = inspect.signature(self._preprocess).parameters
        return all(f"{identifier}_list" in params for identifier in self._prompt.get_identifiers())
    ...
    def __call__(self, *args, batch_size: int = 1, aggregate: bool = True, **kwargs) -> Any:
        ...
        for i in range(0, size, batch_size):
            batch_messages_list = [messages_list[batch_indice] for batch_indice in range(i, min(i + batch_size, size))]
