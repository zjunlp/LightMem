        ...
        while response_content is None and counter <= max_tolerance:
            try:
                response = self.chat.completions.create(
                    model=model, messages=messages, temperature=temperature, top_p=top_p, stream=stream, **kwargs
                )
                if stream:
                    chunks = []
                    reasoning_chunks = []
...
def openai_api_batch_inference(
    ...
    **kwargs,
    ...
    with ThreadPoolExecutor(max_workers=n_jobs) as executor:
        futures = [executor.submit(apply_func, clients[i], messages_list[i]) for i in range(n_jobs)]
...
class OpenAIClientPool:
    ...
    def __init__(self, api_keys: List[str] | str, base_urls: List[str] | str, model: str = "gpt-4o", **kwargs) -> None:
        ...
        self.client_pool = [
            OpenAIClient(api_key=api_key, base_url=base_url, **kwargs) for api_key, base_url in zip(api_keys, base_urls)
        ]
        self.model = model

    def __call__(
        self, messages_list: List[List[Dict[str, str]]], post_processor: Optional[Callable[[str], Any]] = None, **kwargs
    ) -> Dict[str, Any] | List[Dict[str, Any]]:
            ...
            if len(batch_clients) == 1:
                ...
                outputs.append(
                    client.get_text_generation_output(
                        batch_messages_list[0], model=self.model, post_processor=post_processor, **kwargs
                    )
                )
            else:
                outputs.extend(
                    openai_api_batch_inference(
                        batch_clients, batch_messages_list, model=self.model, post_processor=post_processor, **kwargs
                    )
                )
...
def get_interface_for_inference(
    model: str, api_keys: Optional[List[str] | str] = None, base_urls: Optional[List[str] | str] = None, **kwargs
    ...
    if api_keys is not None or base_urls is not None:
        raise ValueError("Either both `api_keys` and `base_urls` must be provided, or neither.")
