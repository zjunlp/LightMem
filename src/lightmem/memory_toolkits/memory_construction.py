...
def _normalize_langmem_messages(*args, **kwargs) -> Dict[str, List[Dict[str, str]] | str | float | int]:
    ...
    for message in messages:
        if isinstance(message, SystemMessage):
            normalized_messages.append({"role": "system", "content": message.content})
        elif isinstance(message, HumanMessage):
            normalized_messages.append({"role": "user", "content": message.content})
        elif isinstance(message, AIMessage):
            if message.content is not None and not isinstance(message.content, str):
                raise ValueError(f"The content of the message is not a string: {type(message.content)}.")
...
def _extract_langmem_model(llm_model: str, query_model: Optional[str], *args, **kwargs) -> Tuple[str, Dict[str, Any]]:
...
def _extract_langmem_output(response) -> Dict[str, List[Dict[str, str]] | str | float | int]:
    ...
    return {"messages": _normalize_langmem_messages([[response.generations[0][0].message]])}
...
def memory_construction(
    ...
    rerun: bool = False,
    message_preprocessor: Optional[Callable[[Message | QuestionAnswerPair, Session], Dict[str, Any]]] = None,
    **kwargs,
) -> Dict[str, float]:
            ...
            wrapper=token_monitor(
                ...
                extract_input_dict=lambda *args, **kwargs: {
                    "messages": [
                        ...
                        {"role": "user", "content": kwargs.get("prompt", args[0])},
                    ],
                    "metadata": {
                        "op_type": (
                            "generation"
                ...
                extract_output_dict=lambda result: {"messages": result},
            ),
        )
        specs = [spec]
        ...
        spec = PatchSpec(
            ...
            wrapper=token_monitor(
                extract_model_name=lambda *args, **kwargs: _extract_langmem_model(
                    config.llm_model, config.query_model, *args, **kwargs
                ),
                extract_input_dict=lambda *args, **kwargs: {
                    ...
                    "metadata": {"op_type": "generation, update"},
                },
            ...
            wrapper=token_monitor(
                ...
                extract_input_dict=lambda *args, **kwargs: {
                    ...
                    "metadata": {
                        "op_type": (
                            "generation"
                            if kwargs["messages"][0]["content"].startswith("You are a Personal Information Organizer")
                            else "update"
                        )
                    },
                },
                ...
                extract_output_dict=lambda response: (
                    {"messages": response.get("content", "")} if isinstance(response, dict) else {"messages": response}
                ),
            ),
        )
        specs = [spec]
    ...
    with patch_ctx:
        ...
        for session in trajectory:
            ...
            for message in session:
                ...
                layer.add_message(msg_dict, timestamp=timestamp)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="A script used to evaluate various memory layers on various datasets.")
    parser.add_argument(
        "--memory-type",
    ...
    parser.add_argument("--num-workers", type=int, default=4, help="The number of threads to use for the evaluation.")
    parser.add_argument(
        "--seed",
        type=int,
        default=42,
    ...
    parser.add_argument("--sample-size", type=int, default=None, help="Subset size from dataset.")
    parser.add_argument("--rerun", action="store_true", help="Ignore saved memory; rebuild from scratch.")
    parser.add_argument("--config-path", type=str, default=None, help="Path to JSON config for memory method.")
    parser.add_argument(
    ...
    parser.add_argument(
        "--start-idx", type=int, default=None, help="The starting index of the trajectories to be processed."
    )
    parser.add_argument(
        "--end-idx", type=int, default=None, help="The ending index of the trajectories to be processed."
    )
    parser.add_argument(
        "--tokenizer-path", type=str, default=None, help="The path to the tokenizer (only for backbone model)."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help=("If set, use GPT-4o to generate image captions in datasets that support it (e.g., LoCoMo)."),
    )
    ...
    if os.path.exists(args.token_cost_save_filename + ".json"):
        ...
        for model, state in token_cost.items():
            ...
            if not is_dict:
                ...
                token_cost[model] = {op: CostState.from_dict(cs) for op, cs in state.items()}
    else:
        token_cost = {}
