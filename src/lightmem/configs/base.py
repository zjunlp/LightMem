...
class BaseMemoryConfigs(BaseModel):
    pre_compress: Optional[bool] = Field(
        default=False, description="If True, enable pre-compression; otherwise disable it."
    )
    pre_compressor: Optional[PreCompressorConfig] = Field(
        default=None, description="Configuration for the compress model (active only if pre_compress=True)."
    )
    topic_segment: Optional[bool] = Field(
        default=False, description="If True, enable topic segmentation; otherwise disable it."
    )
    ...
    topic_segmenter: Optional[TopicSegmenterConfig] = Field(
        default=None, description="Configuration for the topic segmenter (active only if topic_segment=True)."
    )
    ...
    index_strategy: Optional[Literal["embedding", "context", "hybrid"]] = Field(
        default=None, description="Indexing strategy to use. Choices: embedding|text|hybrid"
    )
    text_embedder: Optional[TextEmbedderConfig] = Field(
        description="Configuration for the text embedding model", default=None
    )
    ...
    retrieve_strategy: Optional[Literal["context", "embedding", "hybrid"]] = Field(
        default="embedding", description="Retrieving strategy to use. Choices: embedding|context|hybrid"
    )
    ...
    kv_cache: Optional[bool] = Field(
        default=False, description="If True, enable precompute KV cache; otherwise disable it."
    )
    ...
    logging: Optional[LoggingConfig] = Field(default=None, description="Logging configuration for the LightMem system.")
