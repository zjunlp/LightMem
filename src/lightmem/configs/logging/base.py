...
class LoggingConfig(BaseModel):
    ...
    level: Union[int, str] = Field(default=logging.INFO, description="Root logging level")

    format: str = Field(
        default="%(asctime)s - %(name)s - %(levelname)s - %(message)s", description="Log message format string"
    )

    date_format: str = Field(default="%Y-%m-%d %H:%M:%S", description="Date format for timestamps")

    console_enabled: bool = Field(default=True, description="Whether to enable console logging")

    console_level: Union[int, str] = Field(default=logging.INFO, description="Logging level for console output")

    file_enabled: bool = Field(default=False, description="Whether to enable file logging")
    ...
    log_filename_prefix: str = Field(default="lightmem", description="Prefix for auto-generated log filenames")
    ...
    timestamp_format: str = Field(
        default="%Y%m%d_%H%M%S", description="Timestamp format for auto-generated log filenames"
    )

    file_level: Union[int, str] = Field(default=logging.DEBUG, description="Logging level for file output")

    file_mode: Literal["a", "w"] = Field(default="a", description="File open mode")

    file_encoding: str = Field(default="utf-8", description="File encoding")

    logger_levels: Optional[Dict[str, Union[int, str]]] = Field(default=None, description="Per-logger level overrides")
    ...
    force_reconfigure: bool = Field(default=False, description="Remove existing handlers before configuration")
    ...
    def resolve_file_path(self) -> "LoggingConfig":
        ...
        filename = self.log_filename_format.format(prefix=self.log_filename_prefix, timestamp=timestamp)
