...
class ContextRetrieverFactory:
    ...
    def from_config(cls, config: ContextRetrieverConfig):
        ...
        if model_name not in cls._MODEL_MAPPING:
            raise ValueError(
                f"Unsupported retriever model: {model_name}. Supported models are: {list(cls._MODEL_MAPPING.keys())}"
            )
        ...
        try:
            ...
            raise ImportError(f"Could not import retriever class '{class_path}': {str(e)}") from e
        except AttributeError as e:
            raise ImportError(f"Maybe class '{class_name}' not found in module '{module_path}': {str(e)}") from e
        except Exception as e:
            raise ValueError(f"Failed to instantiate {model_name} retriever: {str(e)}") from e
