...
class Qwen3VLModel:
    ...
    def generate_batch(
        self, batch_prompts: List[Union[str, List[Dict[str, Any]]]], text_format: Optional[type] = None, **kwargs
    ) -> List[Any]:
        """
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            results = list(
                executor.map(
                    lambda p: self.generate(self._normalize_prompt(p), text_format=text_format, **kwargs), batch_prompts
                )
            )
    ...
    def __repr__(self) -> str:
        ...
        return f"Qwen3VLModel(model_name='{self.model_name}', kwargs={self.kwargs})"
