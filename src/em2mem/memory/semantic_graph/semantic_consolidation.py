...
class SemanticConsolidation:
    ...
    def find_relevant_triples(
        self, new_triples: List[List[str]], existing_triples: List[Tuple[List[str], List[str]]], top_k: int = 20
    ...
    def consolidate_triple(
        self, new_triple: List[str], relevant_existing_triples: List[List[str]]
        ...
        messages = self.prompt_template_manager.render(
            name="semantic_consolidation", new_triple=new_triple, existing_triples=formatted_existing_triples
        )
    ...
    def batch_semantic_consolidation(
        self,
        existing_semantic_results: Tuple[List[List[str]], List[List[str]]],
        new_semantic_results: Tuple[List[List[str]], List[List[str]]],
    ...
    def _process_timestamp_triples_concurrent(
        ...
        with ThreadPoolExecutor() as executor:
            ...
            pbar = tqdm(as_completed(futures), total=len(futures), desc="Consolidating triples", leave=False)
