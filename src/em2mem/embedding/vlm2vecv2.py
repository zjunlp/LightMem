...
from .VLM2Vec.src.arguments import DataArguments, ModelArguments
from .VLM2Vec.src.model.model import MMEBModel
from .VLM2Vec.src.model.processor import (
    QWEN2_VL,
    VLM_IMAGE_TOKENS,
    VLM_VIDEO_TOKENS,
    Qwen2_VL_process_fn,
...
class VLM2VecV2EmbeddingModel:
    ...
    def encode_image(
        self, images: Union[Image.Image, List[Image.Image], str, List[str]], query_text: Optional[str] = None, **kwargs
    ) -> np.ndarray:
        """
    ...
    def encode_video(
        self,
        video_paths: Union[str, Dict[str, Any], List[str], List[Dict[str, Any]]],
        ...
        **kwargs,
        ...
        with torch.no_grad():
            for video_path in video_paths:
                ...
                inputs = self.processor(text=query_text, videos=video_inputs, return_tensors="pt")
                inputs = {key: value.to(self.device) for key, value in inputs.items()}
    ...
    def get_embedding_dimension(self) -> int:
        ...
        with torch.no_grad():
            test_input = self.processor(text="test", images=None, return_tensors="pt")
