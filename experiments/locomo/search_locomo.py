...
def parse_locomo_dataset(data_path: str) -> List[Dict]:
    ...
    for item in data:
        ...
        for qa_item in item.get("qa", []):
            ...
            sample["qa"].append({"question": qa_item["question"], "answer": answer, "category": qa_item["category"]})
...
def retrieve_by_speaker(
    entries: List[Dict], retriever: VectorRetriever, question: str, limit_per_speaker: int
) -> List[Dict]:
    """
...
def retrieve_combined(entries: List[Dict], retriever: VectorRetriever, question: str, total_limit: int) -> List[Dict]:
    """
...
def retrieve_summaries(summaries: List[Dict], retriever: VectorRetriever, question: str, limit: int) -> List[Dict]:
    """
...
def build_prompt_with_speaker_memories(
    question: str, retrieved_entries: List[Dict], enable_summary: bool = False, summaries: Optional[List[Dict]] = None
) -> str:
    """
...
def process_sample(
    ...
    try:
        ...
        if enable_summary:
            ...
            logger.info(f"[{sample_id}] Loaded {len(entries)} entries + {len(summaries)} summaries")
        else:
    ...
    for qa_idx, qa in enumerate(sample["qa"]):
        ...
        if enable_summary:
            ...
            logger.info(f"[{sample_id}] Retrieved {len(retrieved_entries)} entries in {retrieval_time:.3f}s")
        ...
        try:
            response = llm_client.chat.completions.create(
                model=llm_model, messages=[{"role": "system", "content": user_prompt}], temperature=0.0
            )
        ...
        try:
            label = evaluate_llm_judge(
                question, reference, generated_answer, client_obj=judge_client, model_name=judge_model
            )
            metrics = {"judge_correct": int(label), "judge_response": "CORRECT" if int(label) == 1 else "WRONG"}
            logger.info(f"[{sample_id}] Judge: {'CORRECT' if int(label) == 1 else 'WRONG'}")
        except Exception as e:
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Vector baseline evaluation for LoCoMo dataset")

    # Data paths
    parser.add_argument("--dataset", type=str, default=DEFAULT_DATA_PATH, help="Path to dataset JSON file")
    parser.add_argument("--qdrant-dir", type=str, default=DEFAULT_QDRANT_DIR, help="Path to Qdrant data directory")
    parser.add_argument("--output-dir", type=str, default=DEFAULT_RESULTS_DIR, help="Output directory for results")
    ...
    parser.add_argument(
        "--total-limit", type=int, default=DEFAULT_RETRIEVAL_LIMIT, help="Total retrieval limit (for combined mode)"
    )
    parser.add_argument(
        "--retrieval-mode", type=str, choices=["combined", "per-speaker"], default="combined", help="Retrieval strategy"
    )
    parser.add_argument("--allow-categories", type=int, nargs="+", default=[1, 2, 3, 4], help="Allowed QA categories")
    parser.add_argument(
        "--embedder", type=str, choices=["huggingface", "openai"], default="huggingface", help="Embedding backend"
    )
    parser.add_argument(
    ...
    parser.add_argument("--enable-summary", action="store_true", help="Enable summary retrieval (StructMem mode)")
    parser.add_argument(
        "--summary-limit", type=int, default=5, help="Retrieval limit for summaries (only used if --enable-summary)"
    )

    # LLM configuration
    parser.add_argument("--llm-api-key", type=str, required=True, help="API key for LLM")
    parser.add_argument("--llm-base-url", type=str, required=True, help="Base URL for LLM API")
    parser.add_argument("--llm-model", type=str, required=True, help="LLM model name")
    parser.add_argument("--judge-api-key", type=str, required=True, help="API key for judge")
    parser.add_argument("--judge-base-url", type=str, required=True, help="Base URL for judge API")
    parser.add_argument("--judge-model", type=str, required=True, help="Judge model name")
