...
def parse_args():
    ...
    parser.add_argument(
        "--extraction_mode", type=str, default="flat", choices=["flat", "event"], help="Extraction mode for LightMem"
    )
    parser.add_argument("--enable_summary", action="store_true", help="Whether to generate summaries")
    parser.add_argument(
        "--summary_time_window", type=int, default=3600, help="Time window for summarization (in seconds)"
    )
    parser.add_argument("--summary_top_k_seeds", type=int, default=15, help="Top K seeds for summarization")
...
def load_lightmem(collection_name, api_key, args, base_dir=QDRANT_POST_UPDATE_DIR):
    config = {
        ...
        "memory_manager": {
            "model_name": "openai",
            "configs": {"model": LLM_MODEL, "api_key": api_key, "max_tokens": 16000, "openai_base_url": API_BASE_URL},
        },
...
def collection_entry_count(collection_name, base_dir):
    try:
        ...
        storage_sqlite = os.path.join(base_dir, collection_name, "collection", collection_name, "storage.sqlite")
...
def process_single_sample(sample, api_key, args):
    ...
    if args.extraction_mode == "event":
        prompt_arg = {"factual": LoCoMo_Event_Binding_factual, "relational": LoCoMo_Event_Binding_relational}
    else:
    ...
    try:
        ...
        case_add_prompt = (
            add_memory_stats["llm"]["add_memory"]["prompt_tokens"] - initial_stats["llm"]["add_memory"]["prompt_tokens"]
        )
        case_add_completion = (
            add_memory_stats["llm"]["add_memory"]["completion_tokens"]
            - initial_stats["llm"]["add_memory"]["completion_tokens"]
        )
        ...
        if args.enable_summary:
            ...
            lightmem_for_summary = load_lightmem(
                collection_name=sample_id, api_key=api_key, args=args, base_dir=QDRANT_PRE_UPDATE_DIR
            )
            ...
            case_summarize_prompt = (
                summarize_stats["llm"]["summarize"]["prompt_tokens"]
                - initial_summarize_stats["llm"]["summarize"]["prompt_tokens"]
            )
            case_summarize_completion = (
                summarize_stats["llm"]["summarize"]["completion_tokens"]
                - initial_summarize_stats["llm"]["summarize"]["completion_tokens"]
            )
            ...
            if summary_result:
                if "total_summaries" in summary_result:
                    ...
                    try:
                        summary_entries, _ = lightmem_for_summary.summary_retriever.scroll(limit=1000)
        ...
        case_update_prompt = (
            update_end_stats["llm"]["update"]["prompt_tokens"] - update_start_stats["llm"]["update"]["prompt_tokens"]
        )
        case_update_completion = (
            update_end_stats["llm"]["update"]["completion_tokens"]
            - update_start_stats["llm"]["update"]["completion_tokens"]
        )
        ...
        logger.info(
            f"  ├─ Add:      {add_memory_duration:.2f}s ({add_memory_duration / case_total_duration * 100:.1f}%)"
        )
        if args.enable_summary:
            logger.info(
                f"  ├─ Summary:  {summarize_duration:.2f}s ({summarize_duration / case_total_duration * 100:.1f}%)"
            )
...
def main():
    ...
    for sample in data:
        ...
        if pre_exists and post_exists:
            ...
            if pre_count > 0 and post_count > 0:
                main_logger.info(f"✓ {sample_id}: Complete (pre={pre_count}, post={post_count})")
                continue
    ...
    for idx, sample in enumerate(missing):
        ...
        main_logger.info(
            f"  Sample [{idx}] {sample['sample_id'][:30]}... → API Key [{api_key_idx}] ({api_key[:20]}...)"
        )
    main_logger.info("")
